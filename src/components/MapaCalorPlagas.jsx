// frontend/src/components/MapaCalorPlagas.jsx
...
// Icono pequeño para TODAS las palmas
...
// Icono para palmas con plagas
function createPlagaIcon(nivel) {
  let color = '#e67e22';
  let size = 32;
...
function MapaCalorPlagas() {
  ...
  const fetchData = async () => {
    try {
      ...
      const plagasUrl = `${API_URL}/api/plagas/with-palma`;

      console.log('🌐 Conectando a palmas:', palmasUrl);
      console.log('🌐 Conectando a plagas:', plagasUrl);

      const [palmasRes, plagasRes] = await Promise.all([
        axios.get(palmasUrl, {
      ...
      console.log('🐛 Plagas:', plagasRes.data?.data?.length || 0);
  ...
  const heatmapData = plagas.reduce((acc, item) => {
      ...
      >
        <div>
          ...
          <p style={{ marginTop: '10px' }}>
            Cargando mapa de calor de plagas...
          </p>
        </div>
      </div>
    );
    ...
    <div>
      ...
      >
        <div>
          ...
          <span style={{ marginRight: '15px', color: '#e67e22' }}>
            🐛 {plagas.length} reportes
          </span>
      ...
      >
        ...
        >
          ...
          {/* Círculos de calor difuminados (plagas) */}
          ...
          {/* Marcadores de plagas (con popup) */}
          {plagas.map((item, index) => {
            ...
            return (
              <Marker
                ...
                position={[lat, lng]}
                icon={createPlagaIcon(item.nivel_infestacion)}
              >
                <Popup>
                  <div
                    style={{
                  ...
                  >
                    <h3 style={{ margin: '0 0 8px 0', color: '#e67e22' }}>
                      🐛 {item.plaga || 'Sin nombre'}
                    </h3>
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          borderRadius: '12px',
                          background:
                            item.nivel_infestacion === 'Crítico'
                              ? '#c0392b'
                              : item.nivel_infestacion === 'Alto'
                                ? '#e67e22'
                                : item.nivel_infestacion === 'Medio'
                                  ? '#f1c40f'
                                  : '#2ecc71',
                          color: 'white',
                          fontWeight: 'bold',
                    ...
                    >
                      ...
                      <span>{item.estado_biologico || 'N/A'}</span>
                    </div>
                    {item.observaciones && (
                      <div
                        style={{
...
export default MapaCalorPlagas;
